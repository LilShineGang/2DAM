...
public class App extends Application {
    ...
    // panel lateral para los bloques
    ...
    // panel de niveles, con lista de niveles, botones administración música
    ...
    private LevelsPanel createLevelPanel() {
        ...
        levelspanel.setOnplay(() -> {
            if (this.levels.getSelected() != null) {
                ...
                if ((this.mp == null && this.levels.getSelected().getSound() != null) || (this.mp != null
                        && this.mp.getMedia().getSource() != this.levels.getSelected().getSound())) {
                    this.mp = new MediaPlayer(new Media(
                            new File(this.levels.getSelected().getSound().replace("\\", "//")).toURI().toString()));
    ...
    private MenuBar createMenu() {
        ...
        // Nueva opción del menú: Transformar nivel XML a HTML
        MenuItem transformMenuItem = new MenuItem("Transform to HTML");
        transformMenuItem.setOnAction(actionEvent -> {
            // Abre un diálogo para seleccionar el archivo XML a transformar
            ...
            if (file != null) {
                try {
                    // Carga el nivel desde el archivo XML
                    Level level = Level.load(file);
                    // Genera el código HTML a partir del nivel
                    String html = generateHTML(level);
                    // Crea el archivo HTML en la misma carpeta que el XML
                    File htmlFile = new File(file.getParent(), file.getName().replace(".xml", ".html"));
                    Files.write(Paths.get(htmlFile.getAbsolutePath()), html.getBytes());
                    // Muestra mensaje de éxito
                    Alert alert = new Alert(AlertType.INFORMATION);
                    alert.setTitle("Listo");
                    alert.setHeaderText(null);
                    alert.setContentText("HTML creado: " + htmlFile.getAbsolutePath());
                    alert.showAndWait();
                } catch (Exception ex) {
        ...
        optionsMenu.getItems().add(transformMenuItem);
    ...
    private ArrayList<ImageWithPath> loadBackgroundFromDirectory(String path) {
        ...
        for (File f : file.listFiles()) {
            images.add(new ImageWithPath(f.toURI().toString()));
        }
        return images;
    }

    // Genera un documento HTML a partir de un nivel del juego
    // Crea una página web con toda la información del nivel en formato tabla
    private String generateHTML(Level level) {
        StringBuilder html = new StringBuilder();
        // Cabecera HTML con el título del nivel
        html.append("<html><head><title>Nivel: ").append(level.getName()).append("</title></head><body>");
        // Título principal
        html.append("<h1>Nivel: ").append(level.getName()).append("</h1>");
        // Información general del nivel
        html.append("<p>Tamaño: ").append(level.getSize()).append("</p>");
        html.append("<p>Tiempo: ").append(level.getTime()).append(" segundos</p>");
        html.append("<p>Fondo: ").append(level.getBackgroundImage()).append("</p>");
        html.append("<p>Sonido: ").append(level.getSound()).append("</p>");
        // Tabla con todos los bloques del nivel
        html.append("<h2>Bloques</h2><table border='1'><tr><th>Tipo</th><th>Posición</th></tr>");
        for (ies.pedro.model.Block block : level.getElements()) {
            html.append("<tr><td>").append(block.getType()).append("</td><td>")
                    .append(block.getRectangle()).append("</td></tr>");
        }
